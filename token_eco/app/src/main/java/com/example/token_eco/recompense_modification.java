            ...
            recompenses.getParent().addListenerForSingleValueEvent(new ValueEventListener() {
                ...
                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                    ...
                    if(titres.contains(titreRecompense.getText().toString().toLowerCase().trim())){
                        titreRecompense.setError("Titre déjà utilisé.");
                    }else{
        ...
        eventRecompense = new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                if (!dataSnapshot.exists()){
                    Toast.makeText(getApplicationContext() , "La récompense a été supprimée", Toast.LENGTH_LONG).show();
                    finish();
                    return;
                }
        ...
        btn_photo.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (!getApplicationContext().getPackageManager().hasSystemFeature(PackageManager.FEATURE_CAMERA_ANY)) {
                    Toast.makeText(recompense_modification.this, "Vous n'avez pas de caméra", Toast.LENGTH_SHORT).show();
                } else {
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        if(requestCode == READ_PERMISSION_CODE){
            if(grantResults.length>0 && grantResults[0] == PackageManager.PERMISSION_GRANTED){
                Toast.makeText(recompense_modification.this, "permission accordée", Toast.LENGTH_SHORT).show();
            }else{
                Toast.makeText(recompense_modification.this, "permission refusée", Toast.LENGTH_SHORT).show();
            }
        }
